...
def stream(camera, output_directory='', save=True):
    camera.begin_acquisition()
    fig = plt.figure(1)
    while True:
        #image_result = camera.acquire_image()
        image_result = camera.acquire_image()
        ...
        plt.pause(0.1)
        plt.clf()
        #print(f'{output_directory}\\{time.time()}.jpg')
        if save:
            image_result.image.save(f'{output_directory}\\{time.time()}.jpg', "JPEG")
    plt.close('all')
    camera.end_acq()

if __name__ == '__main__':
    EXPOSURETIME = 10
    output_dir = r"C:\Users\mtm5k\Desktop\astro"
    ...
    fig = plt.figure(1)
    camera.set_exposure(EXPOSURETIME * 1E6)
    stream(camera, output_directory=output_dir)
    camera.begin_acquisition()
